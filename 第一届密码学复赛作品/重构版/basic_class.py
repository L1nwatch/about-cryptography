...
class BasicUI:
    ...
    def _receive_file(self):
        ...
        def __exchange_encrypted_des_key(sock):
            self._update_state_board("开始交换加密的 AES 密钥...", print_sep=True)
        ...
        def __ask_decrypt_file_now(path):
            choice = tkinter.messagebox.askyesno(
                message="是否现在解密文件?[PS: 每次发送的文件都会使用新的 AES 密钥, 故需要在下次发送文件前对接收到的加密文件进行解密]")
            if choice is True:
    ...
    def initialize_root(self):
        ...
        self.root_tk.title("Mini Filer {} v1.0".format(self.my_name))
    ...
    def _decrypt_file(self, encrypted_file=None):
        ...
        def __decrypt_encrypted_file(path):
            ...
            counter = slice(0, 16)
            aes = AES.new(des_key, AES.MODE_CTR, counter=lambda: data[counter])
            decrypt_content = aes.decrypt(data[16:])
    ...
    def _send_file(self):
        def __encrypt_file():
            ...
            encrypt_sir = AES.new(des_key, AES.MODE_CTR, counter=lambda: counter)
        ...
        des_key = __create_des_key(key_size=16)
